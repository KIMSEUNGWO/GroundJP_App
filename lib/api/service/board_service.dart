...
class BoardService extends PipeBuffer<BoardService> {
  ...
  Future<List<BoardSimp>> getBoardList(
      {required Region region, required Pageable pageable}) async {
    String uri =
        '/api/search/board?&page=${pageable.page}&size=${pageable.size}';
  ...
  Future<ResponseResult> postBoard(
  ...
  Future<ResponseResult> patchBoard(
      {required String title,
      required String content,
      int? matchId,
      Region? region,
      required int boardId}) async {
    final body = {'boardId': boardId, 'title': title, 'content': content};
    if (matchId != null) {
      body.addAll({'matchId': '$matchId'});
    }
    if (region != null && region != Region.ALL) {
